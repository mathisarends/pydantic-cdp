...
class ClientGenerator(BaseGenerator):
    ...
    def generate(self, domain: Domain) -> str:
        ...
        methods = tuple(self._build_method(command, ctx) for command in domain.commands)
        ...
        return render_template(
            ...
            command_imports=self._command_imports(domain),
    ...
    def _command_imports(self, domain: Domain) -> tuple[str, ...]:
    ...
    def _build_method(self, command: Command, ctx: GenerationContext) -> MethodView:
        parameters = tuple(
            self._build_parameter_view(command, parameter, ctx)
        ...
        constructor_args = ", ".join(
            f"{to_snake_case(parameter.name)}="
            f"{self._parameter_name(command, parameter)}"
            for parameter in command.parameters
        )
    ...
    def _build_parameter_view(
        self, command: Command, parameter: Parameter, ctx: GenerationContext
    ) -> FieldView:
        ctx.scan_param(parameter)
        annotation = resolve_type(parameter)
        ctx.track_type_string(annotation)
        return FieldView(
            name=self._parameter_name(command, parameter),
            annotation=annotation,
            optional=parameter.optional,
        )

    def _parameter_name(self, command: Command, parameter: Parameter) -> str:
